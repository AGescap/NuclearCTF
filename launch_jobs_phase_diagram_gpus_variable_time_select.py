...
def launching_jobs():
    ...
    probwaves_amplitude = [1, 2, 3, 4]
    probwaves_offset = [0.99, 1.99, 2.99, 3.99]
    probwaves_frequency = [0.1, 0.5, 1.0]
    ...
    name_py_script = 'myrtle_cifar10_sgd_nesterov_minibatches_piecewise_linear_lr_stop_osc_epoch_foriloop_weight_decay_variabtimeselection.py'
...
##    shutil.copy('density.py',folder_name)
    ...
    for num_period in num_periods_list:
        ...
        f.write('python3 ' + name_py_script + ' --stop_osc ' + stop_osc + ' --stop_osc_at_epoch ' + stop_osc_at_epoch + ' --start_osc_from_the_middle ' + str(num_period)
                + ' --repeat_sim ' + repeat_sim + ' --myrtle_depth ' + myrtle_depth + ' --myrtle_width ' + myrtle_width + ' --loss_function_type ' +loss_function_type
                ...
                + ' --seed_initialization ' + str(seed_initialization) + ' --probwaves_amplitude' + probwaves_amplitude + ' --probwaves_offset' + probwaves_offset 
                + ' --probwaves_frequency ' + probwaves_frequency                
...
##        + ' --type_data_set ' + type_data_set + ' --optimizer_type ' +optimizer_type + ' --momentum_nest ' + momentum_nest
##        + ' --mini_batch_size ' + mini_batch_size + ' --mini_batch_size_acc ' + mini_batch_size_acc
